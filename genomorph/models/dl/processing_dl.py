...
class DLProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        if dna_tokenizer is None:
            # ProcessorMixin validates dna_tokenizer against dna_tokenizer_class and
            # rejects None.  In LLM-only mode there is no DNA tokenizer, so we
            # temporarily remove it from `attributes` to skip the type check.
            _orig_attrs = type(self).attributes
            type(self).attributes = ["tokenizer"]
            try:
                super().__init__(tokenizer, chat_template=chat_template)
            finally:
                type(self).attributes = _orig_attrs
        else:
    ...
    def tokenize_dna_sequences(
        ...
        # In LLM-only mode there is no DNA tokenizer — skip silently
    ...
    def __call__(
        ...
        **kwargs: Unpack[DLProcessorKwargs],
        ...
        if batch_dna_sequences is not None:
            ...
            # Skip when dna_tokenized is None (LLM-only mode or empty batch)

...
from genomorph.trainer import DNALLMGRPOConfig
from genomorph.utils.vllm_utils import should_update_and_canonicalize, fix_param_name_to_vllm, sync_fsdp1_params_to_vllm, sync_fsdp2_params_to_vllm
...
class DNALLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.max_clip_loss_weight = args.max_clip_loss_weight
        ...
        if self.beta == 0.0:
            ...
            self.ref_model = None
        elif is_peft_model(model) or is_peft_model(getattr(model, "text_model", None)):
            # If PEFT is used (directly or on model.text_model), the reference model is not needed
            # since the adapter can be disabled to revert to the SFT-merged base model.
            self.ref_model = None
        else:
    ...
    def _generate_and_score_completions(self, inputs: Dict[str, Union[torch.Tensor, Any]]) -> Dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.use_vllm:
            ...
            with (
                ...
                start = time.time()
                if self.args.use_hrpo_gate:
                    prompt_completion_ids = unwrapped_model.generate_with_hrpo_gate(
                        input_ids=kwargs["input_ids"],
                        attention_mask=kwargs.get("attention_mask"),
                        dna_tokenized=kwargs.get("dna_tokenized"),
                        batch_idx_map=kwargs.get("batch_idx_map"),
                        max_latent_steps=self.args.max_latent_steps,
                        min_latent_steps=self.args.min_latent_steps,
                        gate_threshold=self.args.gate_threshold,
                        generation_config=self.generation_config,
                    )
                elif self.args.use_latent_reasoning:
                    prompt_completion_ids = unwrapped_model.generate_with_latent_reasoning(
                        input_ids=kwargs["input_ids"],
                        dna_tokenized=kwargs.get("dna_tokenized"),
                        batch_idx_map=kwargs.get("batch_idx_map"),
                        num_latent_steps=self.args.num_latent_steps,
                        generation_config=self.generation_config,
                    )
                else:
        ...
        # Log prompt and completion texts (strip im_end from display)
        _display_completions = [t.replace("<|im_end|>", "").replace("<|im_start|>", "").strip() for t in completions_text]
        self._logs["prompt"].extend(gather_object(prompts_text))
        self._logs["completion"].extend(gather_object(_display_completions))
    ...
    def _compute_clip_loss(self, model, prompt_ids, prompt_mask,
                           completion_ids, completion_mask, multimodal_inputs):
        """
        CLIP-style contrastive loss aligning DNA embeddings to answer hidden state shifts.

        u_dna   = mean-pooled projected DNA embeddings per batch item
        delta_h = mean(answer hidden states) - mean(prompt hidden states)

        L_CLIP = symmetric cross-entropy on the cosine similarity matrix between
                 u_dna and delta_h, pushing matched pairs together and
                 unmatched pairs apart.
        """
        ...
        # ── Get projected DNA embeddings (gradient flows through dna_projection / CrossAttentionFusion) ──
        batch_dna_embeds = unwrapped.process_dna_embeddings(
            dna_tokenized, batch_idx_map, batch_size
        )
        # Combine reference + variant embeddings → one vector per batch item
        u_dna_list = []
        hidden_size = unwrapped.text_hidden_size
        device = prompt_ids.device
        dtype = next(unwrapped.dna_projection.parameters()).dtype
        for i in range(batch_size):
            parts = []
            # slot i = first DNA sequence, slot i+batch_size = second DNA sequence
            ...
            if parts:
                ...
                u_dna_list.append(torch.zeros(hidden_size, device=device, dtype=dtype))
        u_dna = torch.stack(u_dna_list).to(device)   # [B, H]

        # ── Get answer hidden state shift (no gradient — stable target) ──
        input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)
        prompt_len = prompt_ids.shape[1]

        with torch.no_grad():
            inputs_embeds, attn_mask = unwrapped.get_prompt_embeddings(
                input_ids=input_ids,
                attention_mask=attention_mask,
                dna_tokenized=dna_tokenized,
                batch_idx_map=batch_idx_map,
        ...
        prompt_hidden = hidden[:, :prompt_len, :].mean(dim=1)  # [B, H]
        answer_hidden = hidden[:, prompt_len:, :].mean(dim=1)  # [B, H]
        delta_h = (answer_hidden - prompt_hidden).to(u_dna.dtype)  # [B, H]

        # ── Symmetric CLIP contrastive loss ──
        u_norm = F.normalize(u_dna.float(), dim=-1)      # [B, H]
        d_norm = F.normalize(delta_h.float(), dim=-1)    # [B, H]
        sim = u_norm @ d_norm.T / self.clip_temperature  # [B, B]
        labels = torch.arange(batch_size, device=device)
        l_clip = (F.cross_entropy(sim, labels) + F.cross_entropy(sim.T, labels)) / 2

        # Mean pairwise cosine similarity (diagonal of un-scaled sim matrix)
        # Used by caller to compute adaptive weight
        mean_sim = (u_norm * d_norm).sum(dim=-1).mean().detach()  # scalar ∈ [-1, 1]

        return l_clip, mean_sim
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Compute final GRPO loss
        loss = ((per_token_loss * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()

        # ── L_CLIP: DNA-answer contrastive alignment ──────────────────────── #
        if (self.max_clip_loss_weight > 0.0
                and multimodal_inputs.get("dna_tokenized") is not None
                and model.training):
            l_clip, mean_sim = self._compute_clip_loss(
                model, prompt_ids, prompt_mask,
                completion_ids, completion_mask,
                multimodal_inputs,
            )

            # Adaptive weight = cosine schedule × (1 - alignment)
            # schedule:   1.0 → 0.0 over training  (fade out as training ends)
            # alignment:  1.0 → 0.0 as sim rises   (fade out as DNA aligns)
            progress = (self.state.global_step / max(self.args.max_steps, 1))
            ...
            self._metrics[mode]["clip_loss"].append(
                self.accelerator.gather_for_metrics(l_clip.detach()).mean().item()
            )
            self._metrics[mode]["clip_weight"].append(effective_clip_weight)
            self._metrics[mode]["clip_mean_sim"].append(mean_sim.item())

        # ── L_gate: HRPO gate regularization ─────────────────────────────── #
        if self.args.use_hrpo_gate and model.training:
            _unwrapped = model.module if hasattr(model, "module") else model
            l_gate = _unwrapped.compute_gate_loss(
                gate_reg_weight=self.args.gate_reg_weight
            )
            loss = loss + l_gate
            self._metrics[mode]["gate_loss"].append(
                self.accelerator.gather_for_metrics(l_gate.detach()).mean().item()
            )
            self._metrics[mode]["gate_steps"].append(
                float(getattr(_unwrapped, "_gate_steps_taken", 0))
            )
            self._metrics[mode]["gate_latent_steps"].append(
                float(getattr(_unwrapped, "_gate_latent_steps", 0))
            )
            # Clear stored hidden states so they aren't reused across accumulation steps
            # (each accumulation sub-batch should only contribute one gate loss term)
            _unwrapped._gate_hidden_states = []
        ...
        # Gradient anomaly guard: skip update on loss spikes to prevent weight corruption
        if torch.isnan(loss) or torch.isinf(loss) or loss.item() > 1e4:
            print(f"[WARNING] Anomalous loss {loss.item():.4e} at step "
                  f"{self.state.global_step} — zeroing gradient to skip update")
            return loss * 0.0
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        # The base Trainer.prediction_step expects inputs to be a dict and calls
        # inputs.get(k), but our DNA collation returns a list for eval batches.
        # GRPO eval is handled by _generate_and_score_completions; skip standard
        # prediction_step to avoid the AttributeError.

...
class DNALLMGRPOConfig(TrainingArguments):
    ...
    _VALID_DICT_FIELDS = (
        getattr(TrainingArguments, "_VALID_DICT_FIELDS", []) + ["model_init_kwargs"]
    )
    ...
    # DDP settings — must be False: Evo2 is frozen so transformers auto-detects
    # unused params and sets find_unused_parameters=True, which conflicts with
    # _set_static_graph() and causes expect_autograd_hooks_ assertion failures.
    # With False, DDP reduces all trainable param gradients unconditionally.
    ddp_find_unused_parameters: bool = field(
        default=False,
        metadata={"help": "Set find_unused_parameters=False in DDP. Must stay False."},
    )
    ...
    max_clip_loss_weight: float = field(
        default=0.0,
        metadata={
            "help": "Maximum weight of the CLIP contrastive loss. The effective weight is "
            "scaled adaptively by a cosine annealing schedule and the current DNA-answer "
            "cosine similarity. Set to 0.0 to disable. Typical values: 0.05–0.2."
        },
    )
    clip_temperature: float = field(
        default=0.07,
        metadata={"help": "Temperature for the CLIP contrastive loss similarity matrix."},
    )
    ...
    no_repeat_ngram_size: int = field(
        default=0,
        metadata={
            "help": "If set to a positive integer, all ngrams of size `no_repeat_ngram_size` can only occur once in "
            "the generation. Setting this to 0 (default) disables ngram blocking."
        },
    )
    ...
    # Parameters that control latent reasoning (Coconut-style)
    use_latent_reasoning: bool = field(
        default=False,
        metadata={
            "help": "If True, use latent (Coconut-style) reasoning instead of <think> token reasoning. "
            "The model performs num_latent_steps recurrence steps in hidden-state space before "
            "generating the final answer."
        },
    )
    num_latent_steps: int = field(
        default=4,
        metadata={
            "help": "Number of latent hidden-state recurrence steps when use_latent_reasoning=True."
        },
    )

    # Parameters that control HRPO adaptive gate (Week 3)
    use_hrpo_gate: bool = field(
        default=False,
        metadata={"help": "Enable HRPO adaptive gate (GateNet) for latent reasoning."},
    )
    use_dna_gate: bool = field(
        default=False,
        metadata={"help": "Condition the GateNet on DNA summary vectors (requires use_hrpo_gate)."},
    )
    max_latent_steps: int = field(
        default=8,
        metadata={
            "help": "Maximum number of latent steps when use_hrpo_gate=True (safety ceiling)."
        },
    )
    min_latent_steps: int = field(
        default=1,
        metadata={
            "help": "Minimum latent steps always taken regardless of gate value. "
            "Forces at least one latent context step so the LLM trains to use hidden-state input."
        },
    )
    gate_threshold: float = field(
        default=0.5,
        metadata={
            "help": "Gate value below which the HRPO loop stops early. "
            "Lower values allow more reasoning steps before stopping."
        },
    )
    gate_reg_weight: float = field(
        default=0.01,
        metadata={
            "help": "Weight for the gate regularization loss L_gate = gate_reg_weight * mean(gate_values). "
            "Encourages early stopping unless the reward justifies more steps."
        },
    )

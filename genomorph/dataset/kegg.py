...
# Common biology terms that look like gene names but aren't
_NON_GENE_TERMS = {
    "ATP", "ADP", "AMP", "GTP", "GDP", "GMP", "DNA", "RNA", "PI3K",
    "IP3", "DAG", "ROS", "NO", "Ca2", "NAD", "NADH", "FADH",
}

# Matches uppercase-only identifiers (gene names): e.g. SNCA, PDE11A, PRKAR1A
_GENE_PATTERN = re.compile(r'\b([A-Z][A-Z0-9]{1,})\b')


def _build_gene_map(text: str) -> Dict[str, str]:
    """
    Scan text for gene names and return a mapping {gene_name: GENE_N}.
    Order of first appearance determines the index.
    """
    ...
    def collect(match: re.Match) -> str:
        ...
        if name not in _NON_GENE_TERMS and name not in gene_map:
            ...
            counter[0] += 1
        return match.group(0)  # no replacement yet, just scanning
...
def _apply_gene_map(text: str, gene_map: Dict[str, str]) -> str:
    """Apply a pre-built gene_map to text, replacing all known gene names."""
    def replace(match: re.Match) -> str:
        return gene_map.get(match.group(1), match.group(1))
    return _GENE_PATTERN.sub(replace, text)


def load_kegg_from_anon_csv(csv_path: str) -> DatasetDict:
    """
    Load a pre-built anonymized KEGG CSV (from build_anon_dataset.py) and
    return a DatasetDict with train/val/test splits.

    The anonymized columns are exposed as 'question' and 'reasoning' so the
    rest of the pipeline (get_format_kegg_function, collate_fn) is unchanged.
    """
...
def _format_kegg(example: Dict[str, Any], model_name: str, is_sft: bool) -> Dict[str, Any]:
    ...
    When loading from an anonymized CSV, 'question' and 'reasoning' already
    contain the anonymized text — no extra processing needed here.
    """
...
def qwen_dna_collate_fn(
    ...
    # Also mask padding tokens (use attention_mask, not pad_token_id comparison,
    # because pad_token_id == eos_token_id == <|im_end|> == 151645, which also
    # appears as a content token inside the assistant response)
    ...
    if truncate_for_generation:
        ...
        for i, k in enumerate(keep_lens):
            ...
            new_input_ids[i, -total:] = src_ids
            new_attention[i, -total:] = src_attn
            new_labels[i, -total:]    = src_lbls

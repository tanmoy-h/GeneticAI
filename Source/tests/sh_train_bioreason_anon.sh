...
#SBATCH --mem=160gb
#SBATCH --output=Source/tests/logs/train_bioreason_anon_%j.out
#SBATCH --error=Source/tests/logs/train_bioreason_anon_%j.err
...
LOG=tests/logs/train_bioreason_anon_$(date +%Y%m%d_%H%M%S).log
exec > >(tee "$LOG") 2>&1
set -x
echo "Command:      bash $0 $*"
echo "Logging:      $LOG"

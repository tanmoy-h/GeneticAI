...
#SBATCH --output=test/anon/logs/test_06_eval_stage3_%j.out
#SBATCH --error=test/anon/logs/test_06_eval_stage3_%j.err
...
LOG=test/anon/logs/test_06_eval_stage3_$(date +%Y%m%d_%H%M%S).log
exec > >(tee "$LOG") 2>&1
echo "Command:        bash $0 $*"
echo "Logging to:     $LOG"

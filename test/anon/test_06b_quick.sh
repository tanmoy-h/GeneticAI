...
#SBATCH --output=test/anon/logs/test_06b_quick_%j.out
#SBATCH --error=test/anon/logs/test_06b_quick_%j.err
...
GATE_CKPT_DIR=${GATE_CKPT_DIR:-/scratch/tanmoyh_iitp/GenoMorph/checkpoints/train_04_stage1_51/s04_pass01}
...
OUTPUT_DIR=${OUTPUT_DIR:-/scratch/tanmoyh_iitp/GenoMorph/checkpoints/test_06b_quick_anon}
## ─────────────────────────────────────────────────────────────────────────────
...
LOG=test/anon/logs/test_06b_quick_$(date +%Y%m%d_%H%M%S).log
exec > >(tee "$LOG") 2>&1
echo "Command:     bash $0 $*"
echo "Logging to:  $LOG"

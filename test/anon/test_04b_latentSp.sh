...
#SBATCH --output=test/anon/logs/test_04b_latentSp_%j.out
#SBATCH --error=test/anon/logs/test_04b_latentSp_%j.err
...
LOG=test/anon/logs/test_04b_latentSp_$(date +%Y%m%d_%H%M%S).log
mkdir -p test/anon/logs
exec > >(tee "$LOG") 2>&1
echo "Command:     bash $0 $*"
echo "Logging to:  $LOG"

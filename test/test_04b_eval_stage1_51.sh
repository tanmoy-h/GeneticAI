#!/bin/bash
#SBATCH --job-name=w9_eval_stage1_51
#SBATCH --gres=gpu:4
#SBATCH --mem=120G
#SBATCH --time=04:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=test/logs/test_04b_eval_stage1_51_%j.out
#SBATCH --error=test/logs/test_04b_eval_stage1_51_%j.err
...
##
## Usage (direct):
##   bash test/test_04b_eval_stage1_51.sh
##   GPUS=0,1 N_SAMPLES=100 bash test/test_04b_eval_stage1_51.sh
##   S=4 GPUS=0 bash test/test_04b_eval_stage1_51.sh
##
## Usage (SLURM):
##   sbatch test/test_04b_eval_stage1_51.sh
...
RESULTS_JSON=${RESULTS_JSON:-test/logs/test_04b_eval_stage1_51_results_${_TS}.json}
RAW_CSV=${RAW_CSV:-test/logs/test_04b_eval_stage1_51_raw_${_TS}.csv}
## ─────────────────────────────────────────────────────────────────────────────
...
if [ -z "${STAGE1_CKPT:-}" ]; then
    for _F in "$CKPT_DIR/stage1_ckpt.txt" \
              /scratch/tanmoyh_iitp/GenoMorph/checkpoints/train_03b_stage1_50_cached/stage1_ckpt.txt; do
        if [ -f "$_F" ]; then
            STAGE1_CKPT=$(cat "$_F")
...
LOG=test/logs/test_04b_eval_stage1_51_$(date +%Y%m%d_%H%M%S).log
exec > >(tee "$LOG") 2>&1
echo "Command:       bash $0 $*"
echo "Logging to:    $LOG"

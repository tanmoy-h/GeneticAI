#!/bin/bash
#SBATCH --job-name=biomedgpt_eval_anon
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH --time=06:00:00
#SBATCH --cpus-per-task=4
#SBATCH --output=testing/logs/run_eval_biomedgpt_anon_%j.out
#SBATCH --error=testing/logs/run_eval_biomedgpt_anon_%j.err
...
##
## Usage:
##   bash testing/run_eval_biomedgpt_anon.sh
##   RESUME=1 bash testing/run_eval_biomedgpt_anon.sh
##   sbatch testing/run_eval_biomedgpt_anon.sh
...
OUT_CSV=testing/logs/biomedgpt_anon_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_biomedgpt_anon_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/biomedgpt_anon_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

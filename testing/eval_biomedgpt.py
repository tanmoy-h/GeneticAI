...
BioMedGPT (PharMolix/BioMedGPT-LM-7B) via local HuggingFace inference.

Usage:
    python testing/eval_biomedgpt.py \
        --csv  genomorph/dataset/global_stage1_anon_genes_mol_keep_chr.csv \
        --out  testing/logs/biomedgpt_results.csv \
        --splits test val
...
def parse_args():
    ...
    p.add_argument("--out",     default="testing/logs/biomedgpt_results.csv")
    p.add_argument("--splits",  nargs="+", default=["test", "val"])
    p.add_argument("--model",   default="PharMolix/BioMedGPT-LM-7B",

...
#SBATCH --output=testing/logs/run_eval_gemini_anon_%j.out
#SBATCH --error=testing/logs/run_eval_gemini_anon_%j.err
...
##
## Usage:
##   GEMINI_API_KEY=AIza... bash testing/run_eval_gemini_anon.sh
##   RESUME=1 GEMINI_API_KEY=AIza... bash testing/run_eval_gemini_anon.sh
##   sbatch testing/run_eval_gemini_anon.sh
...
OUT_CSV=testing/logs/gemini_anon_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_gemini_anon_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/gemini_anon_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

#!/bin/bash
#SBATCH --job-name=gpt4o_mini_eval_hf
#SBATCH --mem=8G
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=2
#SBATCH --output=testing/logs/run_eval_gpt4o_mini_hf_%j.out
#SBATCH --error=testing/logs/run_eval_gpt4o_mini_hf_%j.err
...
##
## Usage:
##   OPENAI_API_KEY=sk-... bash testing/run_eval_gpt4o_mini_hf.sh
##   RESUME=1 OPENAI_API_KEY=sk-... bash testing/run_eval_gpt4o_mini_hf.sh
##   sbatch testing/run_eval_gpt4o_mini_hf.sh
...
OUT_CSV=testing/logs/gpt4o_mini_hf_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_gpt4o_mini_hf_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/gpt4o_mini_hf_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

#!/bin/bash
#SBATCH --job-name=claude_eval_anon
#SBATCH --mem=8G
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=2
#SBATCH --output=testing/logs/run_eval_claude_anon_%j.out
#SBATCH --error=testing/logs/run_eval_claude_anon_%j.err
...
##
## Usage:
##   ANTHROPIC_API_KEY=sk-ant-... bash testing/run_eval_claude_anon.sh
##   RESUME=1 ANTHROPIC_API_KEY=sk-ant-... bash testing/run_eval_claude_anon.sh
##   sbatch testing/run_eval_claude_anon.sh
...
OUT_CSV=testing/logs/claude_anon_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_claude_anon_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/claude_anon_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

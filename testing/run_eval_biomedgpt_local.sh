...
#SBATCH --output=testing/logs/run_eval_biomedgpt_local_%j.out
#SBATCH --error=testing/logs/run_eval_biomedgpt_local_%j.err
...
##
## Usage:
##   bash testing/run_eval_biomedgpt_local.sh
##
##   # Only test split:
##   SPLITS="test" bash testing/run_eval_biomedgpt_local.sh
##
##   # Resume an interrupted run:
##   RESUME=1 bash testing/run_eval_biomedgpt_local.sh
##
##   # Specific GPU:
##   CUDA_VISIBLE_DEVICES=1 bash testing/run_eval_biomedgpt_local.sh
##
##   # SLURM:
##   sbatch testing/run_eval_biomedgpt_local.sh
...
OUT_CSV=testing/logs/biomedgpt_local_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_biomedgpt_local_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/biomedgpt_local_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

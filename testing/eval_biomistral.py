...
BioMistral (BioMistral/BioMistral-7B) via local HuggingFace inference.

Usage:
    python testing/eval_biomistral.py \
        --csv  genomorph/dataset/global_stage1_anon_genes_mol_keep_chr.csv \
        --out  testing/logs/biomistral_results.csv \
        --splits test val
...
def build_prompt(tokenizer, user_msg: str) -> str:
    # BioMistral uses Mistral's [INST] format; apply_chat_template handles it.
    messages = [
...
def parse_args():
    ...
    p.add_argument("--out",     default="testing/logs/biomistral_results.csv")
    p.add_argument("--splits",  nargs="+", default=["test", "val"])
    p.add_argument("--model",   default="BioMistral/BioMistral-7B",

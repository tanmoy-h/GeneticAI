#!/bin/bash
#SBATCH --job-name=biomistral_eval_hf
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH --time=06:00:00
#SBATCH --cpus-per-task=4
#SBATCH --output=testing/logs/run_eval_biomistral_hf_%j.out
#SBATCH --error=testing/logs/run_eval_biomistral_hf_%j.err

## Baseline evaluation of the 290 held-out records (test + val) with BioMistral.
## Dataset: wanglab/kegg (real gene names, HuggingFace)
##
## Usage:
##   bash testing/run_eval_biomistral_hf.sh
##   RESUME=1 bash testing/run_eval_biomistral_hf.sh
##   sbatch testing/run_eval_biomistral_hf.sh
...
OUT_CSV=testing/logs/biomistral_hf_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_biomistral_hf_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/biomistral_hf_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

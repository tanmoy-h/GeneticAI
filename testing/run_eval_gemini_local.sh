#!/bin/bash
#SBATCH --job-name=gemini_eval_local
#SBATCH --mem=8G
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=2
#SBATCH --output=testing/logs/run_eval_gemini_local_%j.out
#SBATCH --error=testing/logs/run_eval_gemini_local_%j.err
...
##
## Usage:
##   GEMINI_API_KEY=AIza... bash testing/run_eval_gemini_local.sh
##
##   # Only test split:
##   SPLITS="test" bash testing/run_eval_gemini_local.sh
##
...
##   sbatch testing/run_eval_gemini_local.sh
...
if [ -z "${GEMINI_API_KEY:-}" ]; then
    ...
    echo "Usage: GEMINI_API_KEY=AIza... bash testing/run_eval_gemini_local.sh"
    exit 1
fi
...
OUT_CSV=testing/logs/gemini_local_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_gemini_local_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/gemini_local_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

#!/bin/bash
#SBATCH --job-name=biomistral_eval
#SBATCH --gres=gpu:1
#SBATCH --mem=32G
#SBATCH --time=06:00:00
#SBATCH --cpus-per-task=4
#SBATCH --output=testing/logs/run_eval_biomistral_%j.out
#SBATCH --error=testing/logs/run_eval_biomistral_%j.err
...
##
## Usage:
##   bash testing/run_eval_biomistral.sh
##
##   # Only test split:
##   SPLITS="test" bash testing/run_eval_biomistral.sh
##
##   # Resume an interrupted run:
##   RESUME=1 bash testing/run_eval_biomistral.sh
##
##   # Specific GPU:
##   CUDA_VISIBLE_DEVICES=1 bash testing/run_eval_biomistral.sh
##
##   # SLURM:
##   sbatch testing/run_eval_biomistral.sh
...
OUT_CSV=testing/logs/biomistral_${TIMESTAMP}.csv
LOG=testing/logs/run_eval_biomistral_${TIMESTAMP}.log
...
RESUME_FLAG=""
if [ "${RESUME:-0}" = "1" ]; then
    LAST_CSV=$(ls -t testing/logs/biomistral_*.csv 2>/dev/null | head -1)
    if [ -n "$LAST_CSV" ]; then
        OUT_CSV="$LAST_CSV"
        RESUME_FLAG="--resume"

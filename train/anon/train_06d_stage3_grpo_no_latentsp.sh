#!/bin/bash
#SBATCH --job-name=grpo_no_latentsp_anon
#SBATCH --gres=gpu:2
#SBATCH --mem=160G
...
##
## Identical to train_10_stage3_grpo_no_latentsp.sh but uses the local
## anonymous CSV instead of the named wanglab/kegg HF dataset.
##
## Usage:
##   STAGE1_CKPT=<path/to/stage1_51/model.pt> bash train/anon/train_10_stage3_grpo_no_latentsp.sh [gpu_ids]
##   RESUME=1 STAGE1_CKPT=<path> bash train/anon/train_10_stage3_grpo_no_latentsp.sh

## ── Configuration ─────────────────────────────────────────────────────────────
CONDA_ENV=dna_env
CACHE_DIR=~/.cache/huggingface
WANDB_PROJECT=${WANDB_PROJECT:-dna-grpo-no-latentsp-anon}
WANDB_ENTITY=${WANDB_ENTITY:-iitp-cse}
KEGG_CSV=${KEGG_CSV:-genomorph/dataset/global_stage1_anon_genes_mol_keep_chr.csv}
OUTPUT_DIR=${OUTPUT_DIR:-/scratch/tanmoyh_iitp/GenoMorph/checkpoints/stage3_grpo_no_latentsp_anon}
...
ACCEL_CFG=/tmp/accelerate_ddp_no_latentsp_anon_${$}.yaml
cat > "$ACCEL_CFG" << EOF
...
echo ""
echo "=== Ablation (no LatentSp, anon) complete. ==="
echo "  NOTE: latentSp_theta_low=0.0 disables entropy-gated latent steps"
echo "  ot_distance reward is active (HiRef OT enabled)"

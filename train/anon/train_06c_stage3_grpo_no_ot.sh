#!/bin/bash
#SBATCH --job-name=grpo_no_ot_anon
#SBATCH --gres=gpu:2
#SBATCH --mem=160G
#SBATCH --time=16:00:00
#SBATCH --cpus-per-task=16
#SBATCH --output=train/anon/logs/stage3_grpo_no_ot_anon_%j.out
#SBATCH --error=train/anon/logs/stage3_grpo_no_ot_anon_%j.err

## Ablation: GenoMorph-B without OT reward — anonymous dataset variant.
##
## Identical to train_09_stage3_grpo_no_ot.sh but uses the local anonymous
## CSV instead of the named wanglab/kegg HF dataset.
##
## Usage:
##   STAGE1_CKPT=<path/to/stage1_51/model.pt> bash train/anon/train_09_stage3_grpo_no_ot.sh [gpu_ids]
##   RESUME=1 STAGE1_CKPT=<path> bash train/anon/train_09_stage3_grpo_no_ot.sh

## ── Configuration ─────────────────────────────────────────────────────────────
CONDA_ENV=dna_env
CACHE_DIR=~/.cache/huggingface
WANDB_PROJECT=${WANDB_PROJECT:-dna-grpo-no-ot-anon}
WANDB_ENTITY=${WANDB_ENTITY:-iitp-cse}
KEGG_CSV=${KEGG_CSV:-genomorph/dataset/global_stage1_anon_genes_mol_keep_chr.csv}
OUTPUT_DIR=${OUTPUT_DIR:-/scratch/tanmoyh_iitp/GenoMorph/checkpoints/stage3_grpo_no_ot_anon}
...
LOG=train/anon/logs/stage3_grpo_no_ot_anon_$(date +%Y%m%d_%H%M%S).log
mkdir -p train/anon/logs
exec > >(tee "$LOG") 2>&1
echo "Command:     bash $0 $*"
echo "Logging to:  $LOG"
...
args=(
    ...
    ## LatentSp — learnable theta_low, same as GenoMorph-B
...
ACCEL_CFG=/tmp/accelerate_ddp_no_ot_anon_${$}.yaml
cat > "$ACCEL_CFG" << EOF
...
echo ""
echo "=== Ablation (no OT, anon) complete. ==="

...
#SBATCH --output=train/logs/stage3_grpo_no_ot_anon_%j.out
#SBATCH --error=train/logs/stage3_grpo_no_ot_anon_%j.err
...
LOG=train/logs/stage3_grpo_no_ot_anon_$(date +%Y%m%d_%H%M%S).log
mkdir -p train/logs
exec > >(tee "$LOG") 2>&1
echo "Command:     bash $0 $*"
echo "Logging to:  $LOG"

...
## Step 1 of the self-adaptive RFT pipeline: sample SAMPLE_PASSES completions per TRAIN
## prompt from a healthy GRPO checkpoint (CONTROLLER mode — latents inserted by the entropy
...
##
## Prompts the GRPO checkpoint never answers correctly are backfilled from the KEGG dataset's
## own reasoning + answer (train/rft/gold_traces.sh), not resampled from another model.
##
## Usage:
##   CKPT=<...>/train_06b_stage3_grpo_optB_spsa/checkpoint-1544 \
...
##
## RESUME=1 resumes a dropped run: records stream to <prefix>_traces.part*.jsonl as they
## complete, so a crash keeps all progress; RESUME=1 skips the (index,pass) already done and
## finishes the rest (re-run the SAME command + OUTPUT_PREFIX).
##
...
if [ -z "$CKPT" ]; then
    echo "ERROR: set CKPT=<healthy GRPO checkpoint dir where latents fire, e.g. checkpoint-1544>"
    exit 1
fi
...
echo "Passes:       $SAMPLE_PASSES  temp=$TEMPERATURE top_p=$TOP_P top_k=$TOP_K  resume=$RESUME"
...
echo "=== Next: filter (DUMP_UNCOVERED) -> gold_traces.sh -> filter again. See train/rft/filter_traces.sh ==="

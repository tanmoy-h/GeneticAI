#!/bin/bash
#SBATCH --job-name=eval_latent_ablation
#SBATCH --gres=gpu:1
#SBATCH --mem=120G
#SBATCH --time=02:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=train/logs/eval_latent_ablation_%j.out
#SBATCH --error=train/logs/eval_latent_ablation_%j.err

## Latent ablation — ZERO-training evaluation of a Stage 3 Option-B checkpoint.
##
...
## No optimizer steps run in either pass, so weights never drift — the delta in
## eval_correctness is purely the latent-reasoning contribution.
##
## Runs on a SINGLE GPU (no DDP) so both arms score the identical 50 records.
##
## Usage:
##   CKPT=<.../train_06b_stage3_grpo_optB/checkpoint-N> bash train/train_06e_eval_latent_ablation.sh [gpu_id]
##   STAGE1_CKPT=<base .../stage1_51/model.pt> CKPT=<checkpoint-N> bash train/train_06e_eval_latent_ablation.sh
##   MODE=ablation CKPT=<checkpoint-N> bash train/train_06e_eval_latent_ablation.sh   # run only one arm

## ── Configuration ─────────────────────────────────────────────────────────────
CONDA_ENV=dna_env
CACHE_DIR=~/.cache/huggingface
KEGG_DATASET=${KEGG_DATASET:-wanglab/kegg}
OUTPUT_DIR=${OUTPUT_DIR:-/scratch/tanmoyh_iitp/GenoMorph/checkpoints/train_06b_stage3_grpo_optB}
...
## GRPO checkpoint to evaluate (the with-latent Option-B checkpoint).
CKPT=${CKPT:-${RESUME_FROM_CKPT:-}}
if [ -z "${CKPT:-}" ]; then
    echo "ERROR: CKPT is not set. Point it at a Stage 3 checkpoint dir, e.g."
    echo "  CKPT=$OUTPUT_DIR/checkpoint-1158 bash train/train_06e_eval_latent_ablation.sh"
    exit 1
fi
if [ ! -d "$CKPT" ]; then
...
## Base Stage 1.51 SFT weights (model init before the adapter is loaded).
## Priority: explicit STAGE1_CKPT → deterministic 1.51 pointer → error.
...
if [ -z "${STAGE1_CKPT:-}" ]; then
    echo "ERROR: STAGE1_CKPT not set and no 1.51 pointer found. Set STAGE1_CKPT=<base model.pt>."
    exit 1
fi

## Gate/injector: the eval loads them from the GRPO checkpoint via the manual
## resume block, but pass the 1.51 copies too so construction has a valid source.
...
## SINGLE GPU — deterministic, no DDP sharding of the 50 eval records.
export CUDA_VISIBLE_DEVICES=${1:-0}
export PYTORCH_ALLOC_CONF=expandable_segments:True
export WANDB_MODE=${WANDB_MODE:-disabled}

LOG=train/logs/eval_latent_ablation_$(date +%Y%m%d_%H%M%S).log
exec > >(tee "$LOG") 2>&1
echo "Command:     bash $0 $*"
echo "Logging to:  $LOG"
...
## Shared args (no training-specific knobs; reward_funcs kept minimal so eval
## construction needs neither Stage 2 manifold nor the OT reward).
base_args=(
...
## Gate/injector construction source
...
echo ""
echo "=== Done. Compare eval_correctness across arms (search the log): ==="
echo "    grep -E 'ARM:|eval_only metrics' $LOG"
echo "  baseline − ablation = latent-reasoning contribution (same weights, same $MAX_EVAL_SAMPLES records)."

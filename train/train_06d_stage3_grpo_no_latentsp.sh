#!/bin/bash
#SBATCH --job-name=grpo_no_latentsp
#SBATCH --gres=gpu:2
#SBATCH --mem=160G
#SBATCH --time=16:00:00
#SBATCH --cpus-per-task=16
#SBATCH --output=train/logs/stage3_grpo_no_latentsp_%j.out
#SBATCH --error=train/logs/stage3_grpo_no_latentsp_%j.err

## Ablation: GenoMorph-B without LatentSp (entropy-gated latent steps disabled).
##
## Uses train_grpo_latent_reasoning.py (fixed-theta, no REINFORCE) with
## --latentSp_theta_low 0.0 so latent steps never trigger (entropy > 0 always).
## Gate and HiRef OT reward remain active.
##
## Purpose: isolates the contribution of LatentSp entropy-gated reasoning.
##
## Usage:
##   STAGE1_CKPT=<path/to/stage1_51/model.pt> bash train/train_10_stage3_grpo_no_latentsp.sh [gpu_ids]
##   RESUME=1 STAGE1_CKPT=<path> bash train/train_10_stage3_grpo_no_latentsp.sh

## ── Configuration ─────────────────────────────────────────────────────────────
CONDA_ENV=dna_env
CACHE_DIR=~/.cache/huggingface
WANDB_PROJECT=${WANDB_PROJECT:-dna-grpo-no-latentsp}
WANDB_ENTITY=${WANDB_ENTITY:-iitp-cse}
KEGG_DATASET=${KEGG_DATASET:-wanglab/kegg}
OUTPUT_DIR=${OUTPUT_DIR:-/scratch/tanmoyh_iitp/GenoMorph/checkpoints/stage3_grpo_no_latentsp}
...
## Pick Stage 1.51 starting checkpoint — IDENTICAL logic to train_06b_stage3_grpo_optB.sh
## so this ablation starts from the SAME checkpoint as the main run (isolates only LatentSp).
...
LOG=train/logs/stage3_grpo_no_latentsp_$(date +%Y%m%d_%H%M%S).log
mkdir -p train/logs
exec > >(tee "$LOG") 2>&1
echo "Command:     bash $0 $*"
echo "Logging to:  $LOG"
...
echo "LatentSp:    DISABLED (theta_low=0.0, ablation)"
nvidia-smi
...
args=(
    ...
    ## LatentSp — disabled: theta_low=0.0 means entropy never falls below threshold
    --latentSp_theta_low        0.0
    --latentSp_theta_high       0.0
    --latentSp_max_consec       3
    --latent_lookahead_k        3
    --latentSp_warmup_steps     400
    --latentSp_ramp_steps       400

    ## GRPO — latent_usage reward removed since LatentSp is off
    --num_generations        8
    ...
    --epsilon                0.1
    --max_grad_norm          0.1
    --reward_funcs           xmlcount soft_format correctness completion_quality reasoning_quality ot_distance
...
## Stage 2 manifold (HiRef OT active)
...
ACCEL_CFG=/tmp/accelerate_ddp_no_latentsp_${$}.yaml
cat > "$ACCEL_CFG" << EOF
...
echo ""
echo "=== Ablation (no LatentSp) complete. Check WandB for: ==="
echo "  train/gate_steps        — gate should fire normally"
echo "  train/gate_latent_steps — should be ~0 (LatentSp disabled)"
echo "  ot_distance reward      — active (HiRef OT enabled)"
echo "  NOTE: latentSp_theta_low=0.0 disables entropy-gated latent steps"

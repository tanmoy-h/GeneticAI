#!/bin/bash
#SBATCH --job-name=w9_precompute_dna
#SBATCH --gres=gpu:1
#SBATCH --mem=80G
#SBATCH --time=4:00:00
#SBATCH --cpus-per-task=8
#SBATCH --output=train/logs/train_00_precompute_dna_%j.out
#SBATCH --error=train/logs/train_00_precompute_dna_%j.err
...
KEGG_CSV=${KEGG_CSV:-}
OUTPUT_PATH=${OUTPUT_PATH:-/scratch/tanmoyh_iitp/GenoMorph/cache/dna_embeddings_kegg_2048.pt}
## ─────────────────────────────────────────────────────────────────────────────
...
LOG=train/logs/train_00_precompute_dna_$(date +%Y%m%d_%H%M%S).log
mkdir -p train/logs
exec > >(tee "$LOG") 2>&1
echo "Command:       bash $0 $*"
echo "Logging to:    $LOG"

...
#SBATCH --mem=160gb
#SBATCH --output=src/scripts/logs/test_bioreason_%j.out
#SBATCH --error=src/scripts/logs/test_bioreason_%j.err
...
LOG=scripts/logs/test_bioreason_$(date +%Y%m%d_%H%M%S).log
exec > >(tee "$LOG") 2>&1
set -x
echo "Command:    bash $0 $*"
echo "Logging:    $LOG"

...
#SBATCH --mem=80gb
#SBATCH --output=Source/tests/logs/test_llm_only_%j.out
#SBATCH --error=Source/tests/logs/test_llm_only_%j.err
...
LOG=tests/logs/test_llm_only_$(date +%Y%m%d_%H%M%S).log
exec > >(tee "$LOG") 2>&1
set -x
echo "Command:   bash $0 $*"
echo "Logging:   $LOG"

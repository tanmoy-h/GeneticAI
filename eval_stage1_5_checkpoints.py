"""
eval_stage1_5_checkpoints.py — Parallel evaluation of Stage 1.5 LatentSp checkpoints.

Distributes checkpoints across multiple GPUs (one subprocess per GPU). Each GPU
process loads the model once and evaluates its assigned checkpoints sequentially.
Results are collected and ranked by answer correctness.

Usage — single GPU:
    python eval_stage1_5_checkpoints.py \\
        --stage1_ckpt  /scratch/.../epoch=03-val_loss=0.4292.ckpt \\
        --ckpt_dir     /scratch/.../stage1_5_sft_cachedp2 \\
        --dna_cache    /scratch/.../dna_embeddings_kegg_2048.pt \\
        --gpus         0 --n_samples 50

Usage — 4 GPUs, only s=1,2,3 steps:
    python eval_stage1_5_checkpoints.py \\
        --stage1_ckpt  /scratch/.../epoch=03-val_loss=0.4292.ckpt \\
        --ckpt_dir     /scratch/.../stage1_5_sft_cachedp2 \\
        --dna_cache    /scratch/.../dna_embeddings_kegg_2048.pt \\
        --gpus         0,1,2,3 --s 1,2,3 --n_samples 50
"""
...
def init_base_model(args, device: str) -> DNALLMModel:
    """Build DNALLMModel, load Stage 1 weights, patch DNA cache."""
...
def load_stage15_weights(model: DNALLMModel, ckpt_path: str):
    """Swap in Stage 1.5 weights (plain state dict) — in-place on existing model."""
...
def find_checkpoints(ckpt_dir: str, s_filter: Optional[List[int]]) -> List[str]:
    """
    Glob s??_pass??/model.pt under ckpt_dir.
    If s_filter is given (e.g. [1, 2]), only include paths where sNN is in the set.
    """
...
def main():
    ...
    if not ckpt_paths:
        ...
        raise FileNotFoundError(f"No s??_pass??/model.pt found{hint} in {args.ckpt_dir}")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Parallel evaluation of Stage 1.5 LatentSp checkpoints by answer correctness"
    )
    p.add_argument("--stage1_ckpt",           required=True,
                   help="Stage 1 Lightning .ckpt to initialize model architecture")
    p.add_argument("--ckpt_dir",              required=True,
                   help="Root dir containing s??_pass??/model.pt checkpoints")
    p.add_argument("--s",                     default=None,
                   help="Curriculum steps to evaluate, e.g. '1,2' or '3'. "
                        "Default: all s steps found.")
